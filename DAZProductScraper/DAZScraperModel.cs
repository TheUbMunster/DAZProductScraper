...
{
    public static partial class DAZScraperModel
    {
        //TODO:
        /*
        ...
        public class Config
        {
            ...
            {
                void DelRec(string path)
                {
                    foreach (string subp in Directory.EnumerateDirectories(path))
                    {
                        DelRec(subp);
                    }
                    try
...
contains(concat(' ', normalize-space(@class), ' '), ' box-additional ')]")?.InnerText?.Trim() ?? string.Empty;
        ...
        public static event Action<int, bool> UpdateProgress; //progress (0-100), isATaskRunning
        public static event Action<CancellationTokenSource> BigTask;
        ...
        public static async Task Generate(List<string> ids, bool overwriteExisting = true)
        {
            ...
            BigTask?.Invoke(cs);
            await GenerateData(ids, cs.Token, overwriteExisting);
            BigTask?.Invoke(null);
            cs.Dispose();
        }
            ...
            string loginJs = @"function puppeteerLogin(form)
{
	...
	pass.value = """ + password + @""";
        ...
        private static async Task GoToProductsPage()
        {
            ...
            LogInfo?.Invoke("At product page (you can do stuff now)", 5, false);
        }
            ...
            string getProductIds = @"function getProductIds(root)
{
   ...
   return result;
        ...
        private static async Task GenerateData(List<string> ids, CancellationToken cts, bool overwriteExisting = true)
        {
            UpdateProgress?.Invoke(0, true);
            ...
            using (SemaphoreSlim semaphore = new SemaphoreSlim(fetchConfig.ImageFetchConcurrency, fetchConfig.ImageFetchConcurrency)) //switch this over to config.
            {
                ...
                for (int i = 0; i < ids.Count; i += amountPerSlam)
                {
                    ...
                    for (int j = i; j < (i + amountPerSlam < ids.Count ? i + amountPerSlam : ids.Count); j++)
                    {
                        ...
                        {
                            //stop doing things.
                            LogInfo?.Invoke("Cancelling tasks...", 2, false);
                            await Task.WhenAll(fetches);
                            LogInfo?.Invoke("All tasks cancelled", 1, false);
                            UpdateProgress?.Invoke(0, false);
                            return;
                        }
                        ...
                        fetches.Add(Task.Run(async () =>
                        {
                            ...
                            if (!imageFileMatches.Any() || overwriteExisting) //no images or overwrite
                            {
                                ...
                                if (cts.IsCancellationRequested)
                                {
                                    LogInfo?.Invoke($"aborting product {fetch.productID}", 2, false);
                                    return;
                                }
                                ...
                                if (fetch.imageUrls != null)
                                {
                                    ...
                                    await DAZScraperModel.GenerateImage(fetch.imageUrls, fetch.cleanedProductName + $"_{fetch.productID}", 9, 100, 3);
                                }
                                else if (fetch.base64Image != null)
                                {
                                    //ssr = new Screenshotter.ScreenshotRequest(fetch.cleanedProductName, fetch.base64Image);
                                    await DAZScraperModel.GenerateImage(fetch.base64Image, fetch.cleanedProductName + $"_{fetch.productID}");
                                }
                                else
                                {
                            ...
                            completedJobs++;
                            UpdateProgress?.Invoke((int)(100 * ((float)completedJobs / ids.Count)), true);
                            semaphore.Release();
                        }));
                    }
                    if (i + amountPerSlam >= ids.Count)
                    {
                        continue; //if the next i loop iteration would put us past the end then don't bother waiting 2 more minutes.
                    }
            ...
            LogInfo?.Invoke("All images have been processed (everything is up-to-date)", 1, false);
            UpdateProgress?.Invoke(0, false);
        }
